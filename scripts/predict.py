...
def main() -> None:
    ...
    # Append a placeholder upcoming match (scores unknown).
    ...
    feats = build_league_features(pd.concat([raw, upcoming], ignore_index=True))
    ...
    x_cat_t = torch.tensor([[home_id, away_id, league_id]], dtype=torch.long)
    ...
    with torch.no_grad():
        lam = model(x_num_t, x_cat_t).squeeze(0).cpu().numpy().tolist()

    lam_home, lam_away = float(lam[0]), float(lam[1])
    mat = score_matrix(lam_home, lam_away, max_goals=args.max_goals)
